...
class Requirements(commands.Converter):
    ...
    __slots__ = ["requir", "blacklist", "bypass", "Amari_Level", "Amari_Weekly", "messages"]
    ...
    def __init__(
        ...
        *,
        ...
        self.requir = requir  # roles that are actually required
        ...
        for key, value in items.items():
            if value:
                if isinstance(value, int):
                    final += f"Minimum {key.replace('_', ' ')}: {value:>4}\n"
                    continue
                final += (
                    f"{key.capitalize()}ed Role(s):\t{humanize_list([f'<@&{i}>' for i in value])}\n"
                )
        return final
