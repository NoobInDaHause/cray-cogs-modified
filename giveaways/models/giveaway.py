...
class Giveaway(GiveawayMeta):
    ...
    async def create_embed(self) -> discord.Embed:
        ...
        if not requirements.null and req_str != "":
            embed.add_field(name="Requirement(s):", value=req_str, inline=False)
    ...
    async def end(self, reason=None) -> "EndedGiveaway":
        ...
        if len(w_list) == 0 or winners == 0:
            ...
            await gmsg.channel.send(content="There were no winners.", embed=endembed2)
            if hostdm == True:
                await self.hdm()
        ...
        await gmsg.channel.send(content=f"{w}", embed=endembed)
