...
class Giveaway(GiveawayMeta):
    ...
    async def hdm(self):
        ...
        if host := self.host:
            try:
                ...
                embed.set_footer(text=f"{self.guild.name} | Winners: {self.amount_of_winners}", icon_url=self.guild.icon_url)
                embed.add_field(name="Prize:", value=self.prize, inline=True)
                embed.add_field(name="Guild:", value=self.guild.name, inline=True)
                embed.add_field(name="Winner(s):", value=self.get_winners_str(), inline=False)
                embed.add_field(name="Jump To Giveaway:", value=f"[Click me]({self.jump_url})", inline=False)
        ...
        for winner in winners.keys():
            if winner:
                try:
                    ...
                    embed.add_field(name="Prize:", value=self.prize, inline=True)
                    embed.add_field(name="Guild:", value=self.guild, inline=True)
                    embed.add_field(name="Host:", value=self.host.mention, inline=True)
                    embed.add_field(name="Jump To Giveaway:", value=f"[Click here]({self.jump_url})", inline=False)
                    embed.set_footer(text=f"{self.guild.name} | Winners: {self.amount_of_winners}", icon_url=self.guild.icon_url)
    ...
    async def create_embed(self) -> discord.Embed:
        ...
        embed_footer_icon = settings.embed_footer_icon.format_map(
            Coordinate(host_avatar_url=self.host.avatar_url, server_icon_url=self.guild.icon_url)
        )
        ...
        if self.flags.donor:
            embed.add_field(name="Donator:", value=f"{self.flags.donor.mention}", inline=False)
    ...
    async def verify_entry(self, member: discord.Member):
        if self.flags.no_donor and member.id == (self.flags.donor or self.host).id:
            return False, (
                f"Your entry for a giveaway has been removed.\n"
                "The host has used the `--no-donor` flag which "
                "denies you from joining your own giveaway."
            )
                    ...
                    if isinstance(value, list):
                        for i in value:
                            if key == "blacklist" and i in member.roles:
                                return False, (
                                    f"Your entry for a giveaway has been removed.\n"
                                    "You had a role that was blacklisted from the giveaway."
                                )
    ...
    async def _handle_flags(self):
        ...
        if msg:
            gw_icon = "https://cdn.discordapp.com/attachments/1000751975308197918/1053641769323090031/image_2022-12-17_195552659-removebg-preview.png"
            kwargs["embed"] = discord.Embed(
                description=f"{msg}", color=await self.get_embed_color()
            ).set_author(name="Donator Message", icon_url=gw_icon)
    ...
    async def cancel(self, reason=None) -> "EndedGiveaway":
        if not self.started:
        ...
        if len(w_list) == 0 or winners == 0:
            embed = gmsg.embeds[0]
            embed.description = (
                f"This giveaway was cancelled.\nThere were no winners.\nHosted by: {host.mention}"
            )
            embed.colour = discord.Colour.red()
            embed.set_footer(text=f"Winners: {winners} | Ended at")
            await gmsg.edit(embed=embed, content="**:no_entry_sign: GIVEAWAY CANCELLED :no_entry_sign:**")

            return EndedGiveaway.from_giveaway(self, reason)

        embed: discord.Embed = gmsg.embeds[0]
        embed.colour = discord.Colour.red()
        embed.description = f"This giveaway was cancelled.\nThere were no winners.\nHosted by: {host.mention}"
        embed.set_footer(text=f"Winners: {winners} | Ended at")
        await gmsg.edit(embed=embed, content="**:no_entry_sign: GIVEAWAY CANCELLED :no_entry_sign:**")
    ...
    async def end(self, reason=None) -> "EndedGiveaway":
        ...
        if len(w_list) == 0 or winners == 0:
            ...
            await url_button.send_message(self.bot, self.channel.id, embed=nowinembed, content=w, url_button=button) # await gmsg.reply(embed=nowinembed, content=w)
            if hostdm == True:
                await self.hdm()
        ...
        winembed = discord.Embed(
                title="Giveaway ended",
                url=link,
                description=endmsg.format_map(formatdict),
                timestamp=datetime.now(tz=timezone.utc),
                colour=0x2F3136
            )
        button = url_button.URLButton(
        ...
        await url_button.send_message(self.bot, self.channel.id, embed=winembed, content=w, url_button=button)
    ...
    async def reroll(self, ctx: commands.Context, winners: int = None):
        ...
        if len(entrants) == 0:
            rembed2 = discord.Embed(
                title="Reroll failed",
                url=link,
                description="There weren't enough entrants to determine a winner.",
                timestamp=datetime.now(tz=timezone.utc),
                colour=0x2F3136
            )
            button = url_button.URLButton(
        ...
        rerollembedtitle = ("Giveaway reroll"
                            if winner
                            else "Reroll failed")
        
        rembed = discord.Embed(
                title=rerollembedtitle,
                url=link,
                description=rerollembed,
                timestamp=datetime.now(tz=timezone.utc),
                colour=0x2F3136
            )
        button = url_button.URLButton(
