...
class Giveaways(commands.Cog):
    ...
    async def g(self, ctx: commands.Context):
        ...
        Note: The giveaway cog can select the same person meaning they can win multiple times on one giveaway.
        """
        ...
        *,
        ...
        if not time and not flags.ends_in:
            return await ctx.send(
                "You must specify a time greater than 30 seconds and less than 2 weeks or use the `--ends-at` flag for a more accurate duration."
            )
...
"""
            + (
                """
    > **__Unavailable __** ||~~*--amt*~~||
        Not available do not use this flag!

    > **__Unavailable __** ||~~*--bank*~~||
        Not available do not use this flag!
"""
