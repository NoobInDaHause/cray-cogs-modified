...
class Giveaways(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        try:
            ...
            if isinstance(result, str):
                embed = discord.Embed(
                    title="Entry denied",
                    url=giveaway.jump_url,
                    description=result,
                    color=discord.Color.red(),
                    timestamp=datetime.now(tz=timezone.utc)
                )
                embed.add_field(name="Giveaway Prize:", value=giveaway.prize, inline=False)
                embed.add_field(name="Jump To Giveaway:", value=f"[Click me]({giveaway.jump_url})", inline=False)
                embed.set_footer(text=f"{giveaway.guild.name}", icon_url=payload.member.guild.icon_url)
                ...
                if reactdm:
                    embed = discord.Embed(
                        ...
                        url=giveaway.jump_url,
                        description=f"Your entry has been granted.",
                        color=await giveaway.get_embed_color(),
                        timestamp=datetime.now(tz=timezone.utc)
                    ).set_footer(text=f"{payload.member.guild.name} | Winners: {giveaway.amount_of_winners}", icon_url=payload.member.guild.icon_url)
                    embed.add_field(name="Giveaway Prize:", value=giveaway.prize, inline=False)
                    embed.add_field(name="Jump To Giveaway:", value=f"[Click me]({giveaway.jump_url})", inline=False)
                    try:
                        await payload.member.send(embed=embed)
        ...
        try:
            ...
            if unreactdm:
                embed = discord.Embed(
                    title="Entry removed",
                    url=giveaway.jump_url,
                    description=f"I have detected your reaction was removed from a giveaway.\n"
                    f"It was either removed by someone or you unreacted.\n"
                    f"React again if you think this is a mistake.",
                    color=await giveaway.get_embed_color(),
                    timestamp=datetime.now(tz=timezone.utc)
                ).set_footer(text=f"{giveaway.guild.name} | Winners: {giveaway.amount_of_winners}", icon_url=giveaway.guild.icon_url)
                embed.add_field(name="Giveaway Prize:", value=giveaway.prize, inline=False)
                embed.add_field(name="Jump To Giveaway:", value=f"[Click me]({giveaway.jump_url})", inline=False)
    ...
    async def g_cancel(
        ...
        if isinstance(giveaway, EndedGiveaway):
            return await ctx.send(
                "That giveaway has already ended. You cannot cancel an ended giveaway."
            )

        reason += f" Cancelled prematurely by {ctx.author}"

        try:
            gaw = await giveaway.cancel(reason)
            self.add_to_cache(gaw)

        except Exception as e:
            log.exception(
                f"Error occurred while cancelling a giveaway with message id: {giveaway.message_id}",
                exc_info=e,
            )
            return await ctx.send(
                f"There was an error while cancelling this giveaway: \n {box(e, 'py')}"
            )

        await ctx.tick(message="Giveaway cancelled successfully!")
...
***__Flags:__ ***
    ...
    **Types of flags**
    ...
    > *--no-multiple-winners*/*--nmw* [argless]
...
***__Customization:__ ***
    ...
    > **Giveaway headers**
        The message above the giveaway can also be changed. `{ctx.clean_prefix}gset startheader` and `{ctx.clean_prefix}gset endheader`
    ...
    > **Thumbnail**
        The giveaway embed thumbnail can be customized as per your liking. `{ctx.clean_prefix}gset thumbnail`
    
    > **Colour**
        The giveaway embed colour can be customized as per your liking. `{ctx.clean_prefix}gset colour`.
        """
        )
