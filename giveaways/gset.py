...
class Gset(Giveaways, name="Giveaways"):
    ...
    async def gset_startheader(self, ctx, *, message):
        """
        Set a custom giveaway start header message.
    ...
    async def gset_endheader(self, ctx, *, message):
        """
        Set a custom giveaway end header message.

        This message shows above the ended giveaway embed."""
        settings = await get_guild_settings(ctx.guild.id, False)
        await settings.msg2.set(message)
        await ctx.reply(f"The new giveaway end header message has been set to \n```\n{message}\n```")
    
    @gset.command(name="thumbnail")
    @commands.admin_or_permissions(administrator=True)
    async def gset_thumbnail(self, ctx: commands.Context, *, thumbnail: str = ""):
        """
    ...
    @gset.command(name="tmsg", aliases=["thankmessage"])
    @commands.admin_or_permissions(administrator=True)
    ...
    async def gset_winnerdm_message(self, ctx: commands.Context, *, message: str):
        """
        Change the embed description message that is sent to the winners when the giveaway ends.
        ...
        Example:
            `[p]gset winnderdm message You have won a giveaway!`"""
        settings = await get_guild_settings(ctx.guild.id, False)
    ...
    async def gset_hostdm_message(self, ctx: commands.Context, *, message: str):
        """
        Change the embed description message that is sent to the host when the giveaway ends.
        ...
        Example:
            `[p]gset hostdm message Your giveaway has ended.`"""
        settings = await get_guild_settings(ctx.guild.id, False)
    ...
    @gset.command(name="description")
    async def gset_description(self, ctx: commands.Context, *, description: str):
        """
        ...
        Example:
            `[p]gset description React with {emoji} to enter.
    ...
    async def gset_footer_text(self, ctx: commands.Context, *, text: str = ""):
        ...
        Example:
            `[p]gset footer text Winners: {winners} | Ends at`"""
        settings = await get_guild_settings(ctx.guild.id, False)
        await settings.embed_footer_text.set(text)
        await ctx.send(
            f"The new giveaway embed footer text has been set to \n{box(text, 'py')}"
            if text
            else "The giveaway embed footer text has been removed."
        )

    @gset_footer.command(name="icon")
    async def gset_footer_icon(self, ctx: commands.Context, *, icon: str = ""):
        ...
        Example:
            `[p]gset footer icon {host_avatar_url}`
            `[p]gset footer icon https://somelink.com`"""
        settings = await get_guild_settings(ctx.guild.id, False)
