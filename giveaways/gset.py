...
class Gset(Giveaways, name="Giveaways"):
    ...
    @gset.command(name="footer")
    async def gset_footer(self, ctx: commands.Context, *, text: str = ""):
        """
        Customize the giveaway embed footer text.
        ...
        Example:
            `[p]gset footer Winners: {winners} | Ends at`
        """
        settings = await get_guild_settings(ctx.guild.id, False)
