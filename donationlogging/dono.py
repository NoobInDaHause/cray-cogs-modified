...
class DonationLogging(commands.Cog):
    ...
    async def dono_log(
        ...
        emoji = bank.emoji
        embed = discord.Embed(
            title="***__Donation Added!__***" if action.lower() == "add" else "***__Donation Removed!__***",
            description=f"{emoji} {humanize_number(amount)} was "
            f"{'added to' if action.lower() == 'add' else 'removed from'} {user.mention}'s donations balance.\n",
            color=await ctx.embed_color(),
        )
        ...
        embed.set_footer(
            text=f"Authorized by {ctx.author} (ID: {ctx.author.id})", icon_url=ctx.author.avatar_url
        )
    ...
    async def reset(
        ...
        embed = discord.Embed(
            title="***__Reset!__***",
            description=f"Resetted {user.mention}'s donation bal. Their current donation amount is {emoji} 0",
            color=await ctx.embed_color(),
        )
                ...
                if user.donations != 0:
                    embed.add_field(
                        name=(
                            f"{index}. **{u} ({u.id})**"
                            if (u := user.user)
                            else f"{index}. **User not found in server ({user.user_id})**"
                        ),
    ...
    async def donoset_reset(self, ctx: commands.Context):
        ...
        if response.content.lower() not in ("yes"): # lower() makes everything lowercase to also catch: YeS, YES etc.
