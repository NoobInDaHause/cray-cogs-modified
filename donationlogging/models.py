...
class DonoBank:
    ...
    async def addroles(self, ctx, user: discord.Member):
        ...
        try:
            ...
            roleadded = (
                f"The following roles were added to `{user} ({user.id})`: {humanize_list([f'**{role.name}**' for role in added_roles])}"
                if added_roles
                else ""
            )
            return roleadded
    ...
    async def removeroles(self, ctx, user: discord.Member):
        try:
            ...
            roleadded = (
                f"Removed some donation amount/roles to `{user} ({user.id})`."
                if removed_roles
                else ""
            )
            return roleadded
...
class DonationManager:
    ...
    def __init__(self, bot) -> None:
        ...
        self.config.register_guild(banks={}, default_bank=None)
